...
(function () {
  ...
  if (form && msg) {
    ...
    var TO = 'ljf1253261766@gmail.com';
    form.addEventListener('submit', function (e) {
      e.preventDefault();
      var name = form.name.value.trim();
      var phone = form.phone.value.trim();
      var email = (form.email && form.email.value ? form.email.value.trim() : '');
      var topic = (form.topic && form.topic.value ? form.topic.value.trim() : '');
      var message = form.message.value.trim();

      if (!name || !phone || !message) {
        msg.className = 'form-msg err';
        msg.textContent = isEn
          ? 'Please fill the required fields (*): Name, Phone, Message.'
          : '请填写带 * 的必填项（称呼、联系电话、需求描述）。';
        return;
      }
      if (!/^1\d{10}$/.test(phone)) {
        msg.className = 'form-msg err';
        msg.textContent = isEn
          ? 'Phone number looks incorrect. Please enter an 11-digit mobile number.'
          : '联系电话格式有误，请填写 11 位手机号。';
        return;
      }

      var subject, body;
      if (isEn) {
        subject = '[Jinmai Inquiry] ' + (topic || 'General') + ' - ' + name;
        body = 'Name: ' + name + '\n' +
               'Phone: ' + phone + '\n' +
               'Email: ' + (email || '-') + '\n' +
               'Inquiry Type: ' + (topic || '-') + '\n' +
               'Message:\n' + message + '\n\n' +
               '-- Sent from the Suqian Jinmai website contact form';
      } else {
        subject = '【金麦官网留言】' + (topic || '咨询') + ' - ' + name;
        body = '称呼：' + name + '\n' +
               '联系电话：' + phone + '\n' +
               '邮箱：' + (email || '（未填）') + '\n' +
               '咨询类型：' + (topic || '（未填）') + '\n' +
      ...
      msg.textContent = isEn
        ? 'Your email app has been opened with the message pre-filled. Click "Send" to deliver it to us.'
        : '已为您打开邮件客户端，内容已自动填好。请点击"发送"把留言发给我们。';
      form.reset();
    });
  }
